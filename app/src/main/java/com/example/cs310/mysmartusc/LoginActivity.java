...
//        }
    ...
    private void getEmails() {
        if (mAccount == null) {
            Log.w(TAG, "getEmail: null account");
            return;
        }

        showProgressDialog();
        new GetEmailsTask(this).execute(mAccount);
    }

    protected void onConnectionsLoadFinished(@Nullable List<Message> messages) {
        hideProgressDialog();

        if (messages == null) {
            Log.d(TAG, "getMessages:connections: null");
            mDetailTextView.setText(getString(R.string.emails_fmt, "None"));
            return;
        }

        Log.d(TAG, "getMessages:connections: size=" + messages.size());

        // Get names of all connections
        StringBuilder msg = new StringBuilder();
        for (int i = 0; i < messages.size(); i++) {
            new GetSingleEmailTask(this, messages.get(i).getId()).execute(mAccount);
            //Message message = messages.get(i);
            //msg.append(message.getId());
        }

        // Display names
        mDetailTextView.setText(getString(R.string.emails_fmt, msg.toString()));
    }
    ...
    private static class GetSingleEmailTask extends AsyncTask<Account, Void, Message> {

        private WeakReference<LoginActivity> mActivityRef;
        String mMessageId;

        public GetSingleEmailTask(LoginActivity activity, String messageId) {
            mActivityRef = new WeakReference<>(activity);
            mMessageId = messageId;
        }

        @Override
        protected Message doInBackground(Account... accounts) {
            ...
            try {
                ...
                return mService
                        .users()
                        .messages()
                        .get(credential.getSelectedAccountName(), mMessageId)
                        .execute();
...
//                        .execute();
        ...
        protected void onPostExecute(Message message) {
            ...
            if (mActivityRef.get() != null) {
                ...
                //mActivityRef.get().onConnectionsLoadFinished(messages);
            }
        }
    }

        private static class GetEmailsTask extends AsyncTask<Account, Void, List<Message>> {

            private WeakReference<LoginActivity> mActivityRef;

            public GetEmailsTask(LoginActivity activity) {
                mActivityRef = new WeakReference<>(activity);
            }

            @Override
            protected List<Message> doInBackground(Account... accounts) {
                if (mActivityRef.get() == null) {
                    return null;
                }
